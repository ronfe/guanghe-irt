...
# Given a unit of probDict
# {problemId: {theta: ptheta}}
# return a list of difficulty and distinction
# [difficulty, distinction]
def estimateDD(unitP, guessingIndex):
    theta = unitP.keys()
    ptheta = []
    for each in theta:
        ptheta.append(unitP[each])

    def Ptheta(x, a, b):
        return guessingIndex + (1-guessingIndex)*(1 / (1 + np.exp(-a * (x - b))))

    initialEstimation = np.array([1.0, 0.0])
    result = optimize.curve_fit(Ptheta, np.array(theta), np.array(ptheta), initialEstimation)
...
def calcParameter(problemList, userGroup):
    result = {}
    ...
    # 2.3 get problem difficulty and distinction
    for each in problemList:
        unitP = probDict[each]
        guessingIndex = result[each]['guessingIndex']
        dd = estimateDD(unitP, guessingIndex)
        result[each]['distinction'] = dd[0]
        result[each]['difficulty'] = dd[1]
